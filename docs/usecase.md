...
| -------- | ------------------------------------------------------------------------- |
| 주 액터  | 일반 참여자                                                               |
| 목적     | 새 정산 그룹을 생성하고 공유 가능한 접근 링크를 발급받는다                |
| 선행조건 | 없음                                                                      |
| 후행조건 | 그룹이 OPEN 상태로 생성되고 UUID 링크가 발급된다. PIN은 해시되어 저장된다 |

**주 흐름**

1. 사용자가 온보딩 화면에서 "그룹 생성하기"를 선택한다.
2. 시스템이 그룹명·PIN 입력 폼을 표시한다.
3. 사용자가 그룹명(1~50자)과 PIN(숫자 4자리)을 입력하고 생성을 요청한다.
4. 시스템이 입력값의 형식을 검증한다.
5. 시스템이 UUID v4를 생성하고 PIN을 BCrypt로 해시하여 그룹을 OPEN 상태로 저장한다.
6. 시스템이 UUID가 포함된 접근 링크를 사용자에게 제공한다.

**대안·예외 흐름**

- 4a. 그룹명이 1~50자 범위를 벗어남 → 오류 메시지 표시 후 3단계로 복귀.
- 4b. PIN이 숫자 4자리가 아님 → 오류 메시지 표시 후 3단계로 복귀.
...
1. 지출 등록 완료 후 시스템이 계좌 등록을 안내한다.
2. 사용자가 은행명·계좌번호를 입력하거나 패스(건너뜀)한다.
3. 입력 시 시스템이 저장한다.
...
- \*(공통). 그룹 상태가 SETTLED → 멤버 추가/제거 불가(계좌 수정은 별도 정책).
...
5. 사용자가 지출 내역 히스토리를 요청하면 시스템이 지출 항목별(항목명, 금액, 낸 사람, 날짜, 분담 대상) 목록을 표시한다.
